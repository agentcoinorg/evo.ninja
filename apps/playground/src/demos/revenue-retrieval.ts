...
export async function run(adapter: LlmAdapter) {
  ...
  await pipe(
    cache("google1", () => 
      searchOnGoogle(
        query,
        adapter.context.env.SERP_API_KEY!
      )
    ),
    ...
    async (result: string) => {
      if (!fs.existsSync("../../workspace/db")) {
        const rag = await Rag.standard(adapter.context, "rag-over-google", workspace)
        ...
        return Rag.standard(adapter.context, "rag-over-google", workspace, TextChunker.fixedCharacterLength(result, { chunkLength: 100, overlap: 50}))
      }
    },
