...
const FN_NAME = "createFunction";

export function createFunction(createScriptWriter: () => ScriptWriter): AgentFunction<AgentContext> {
  return {
    definition: {
      name: "createFunction",
        description: `Create a function using JavaScript.`,
        parameters: {
          type: "object",
          properties: {
            namespace: {
            ...
            arguments: {
              type: "string",
              description: "The arguments of the function. E.g. '{ path: { type: \"string\" }, data: { type: \"string\" }'. Use only what you need, no optional arguments."
            },
          },
    ...
    buildChatMessage(args: any, result: AgentFunctionResult): AgentChatMessage {
      ...
      return result.ok
        ? {
            type: "success",
            title: `Created '${args.namespace}' function.`,
            content: 
              `# Function Call:\n\`\`\`javascript\n${FN_NAME}(${argsStr})\n\`\`\`\n` +
    ...
    buildExecutor(context: AgentContext) {
      return async (options: { namespace: string, description: string, arguments: string }): Promise<AgentFunctionResult> => {
        if (options.namespace.startsWith("agent.")) {
          return ResultErr(`Cannot create an function with namespace ${options.namespace}. Try searching for function in that namespace instead.`);
        }

        // Create a fresh ScriptWriter agent
        const writer = createScriptWriter();

        context.logger.notice(`Creating function '${options.namespace}'...`);
