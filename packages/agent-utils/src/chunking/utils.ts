...
export const getTextFromPriorChunks = (args: {
  ...
  for (let i = currentIndex - 1; i >= 0; i--) {
    ...
    const prevResultText = overlap
      ? prevResult.text().slice(0, -overlap)
      : prevResult.text();
...
export const getTextFromNextChunks = (args: {
  ...
  for (let i = currentIndex + 1; i <= sortedElements.length; i++) {
    ...
    const nextResultText = overlap
      ? nextResult.text().slice(overlap)
      : nextResult.text();
...
export const sortDocumentsByIndex = <TMetadata extends BaseDocumentMetadata>(
  documents: LocalDocument<TMetadata>[]
