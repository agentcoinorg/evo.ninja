...
export class DirectoryChunker implements Chunker<DirectoryChunkerInput> {
  ...
  private chunkDirectory(workspace: Workspace, directory: string): string[] {
    const entries = workspace.readdirSync(directory);
    const chunks: string[] = [];
    ...
    for (const entry of entries) {
      ...
      if (entry.type === 'file') {
        const fileContents = workspace.readFileSync(fullPath).split('\n');
        let startLine = 0;
        ...
        while (startLine < fileContents.length) {
          ...
          if (currentSize >= this.config.maxChunkSize || (startLine < fileContents.length && currentSize + chunkHeader.length > this.config.maxChunkSize)) {
        ...
        const subchunks = this.chunkDirectory(workspace, fullPath);
