...
export const requireShim = (lib: string) => {
  const fs = {
    readFileSync: (path: string) => {
      return __wrap_subinvoke("plugin/fs", "readFileSync", clean({ path }))
        .value;
    },
    readFile: (...args: any[]) => {
    ...
    rename: (...args: any[]) => {
      ...
      const result = __wrap_subinvoke(
        "plugin/fs",
        "renameSync",
        clean({ oldPath: args[0], newPath: args[1] })
      );
      callback &&
        callback(
          result.error ? new Error(result.error) : undefined,
    ...
    readdirSync: (path: string) => {
      return __wrap_subinvoke("plugin/fs", "readdirSync", clean({ path }))
        .value;
    },
    readdir: (...args: any[]) => {
      const callback = args[args.length - 1];
  ...
  const axios = {
    get: (url: string, config: any) => {
      return Promise.resolve(
        __wrap_subinvoke("plugin/axios", "get", clean({ url, config })).value
      );
    },
    post: (url: string, data: string, config: any) => {
      return Promise.resolve(
        __wrap_subinvoke("plugin/axios", "post", clean({ url, data, config }))
          .value
      );
    },
    put: (url: string, data: string, config: any) => {
      return Promise.resolve(
        __wrap_subinvoke("plugin/axios", "put", clean({ url, data, config }))
          .value
      );
    },
    delete: (url: string, config: any) => {
      return Promise.resolve(
        __wrap_subinvoke("plugin/axios", "delete", clean({ url, config })).value
      );
    },
    head: (url: string, config: any) => {
      return Promise.resolve(
        __wrap_subinvoke("plugin/axios", "head", clean({ url, config })).value
      );
    },
  };
  ...
  const path = {
    resolve: (pathSegments: any) => {
      return __wrap_subinvoke("plugin/path", "resolve", clean({ pathSegments }))
        .value;
    },
    normalize: (path: string) => {
      return __wrap_subinvoke("plugin/path", "normalize", clean({ path }))
        .value;
    },
    isAbsolute: (path: string) => {
      return __wrap_subinvoke("plugin/path", "isAbsolute", clean({ path }))
        .value;
    },
    join: (path: string) => {
    ...
    relative: (from: string, to: string) => {
      return __wrap_subinvoke("plugin/path", "relative", clean({ from, to }))
        .value;
    },
    dirname: (path: string) => {
    ...
    format: (pathObject: any) => {
      return __wrap_subinvoke("plugin/path", "format", clean({ pathObject }))
        .value;
    },
    parse: (path: string) => {
  ...
  const util = (function () {
    ...
    const getOwnPropertyDescriptors = function getOwnPropertyDescriptors(
      obj: any
    ) {
    ...
    const kCustomPromisifiedSymbol: any = undefined;
  ...
  switch (lib) {
    ...
    default:
      throw new Error(
        `Cannot do require('${lib}'), '${lib}' is an unknown import.`
      );
  }
};
