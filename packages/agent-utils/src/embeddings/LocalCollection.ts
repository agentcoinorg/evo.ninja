...
import {
  getTextFromNextChunks,
  getTextFromPriorChunks,
  sortDocumentsByIndex,
} from "../chunking/utils";
...
> {
  ...
  async add(items: string[], metadatas?: TMetadata[]): Promise<void> {
    ...
    for await (const result of results) {
      const metadata =
        metadatas && metadatas.length > idx ? metadatas[idx] : undefined;
  ...
  async search(
    query: string,
    limit?: number
  ): Promise<LocalDocument<TMetadata>[]> {
    const queryEmbeddingResults =
      await this.embeddingApi.createEmbeddings(query);
    ...
    const scores = documents.map((document) => {
      ...
      const score = normalizedCosineSimilarity(
        queryVector,
  ...
  async searchWithSurroundingContext(
    query: string,
    opts: { surroundingCharacters: number; overlap?: number; limit?: number }
  ) {
  ...
  async searchUnique(
    ...
    const sortedScores = [
      ...new Map(scores.map((x) => [x.text, x])).values(),
