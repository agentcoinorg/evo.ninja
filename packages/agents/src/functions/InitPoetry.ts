...
export class InitPoetryFunction extends AgentFunctionBase<InitPoetryFuncParameters> {
  ...
  buildExecutor({
    ...
    params: InitPoetryFuncParameters,
    rawParams?: string
  ...
  private async poetryInit(
    workspace: Workspace
  ): Promise<{ exitCode: number; stdout: string; stderr: string }> {
    await workspace.exec("poetry", ["init", "-n"]);
    const dependencies = await this.getPythonDependencies(workspace, "/");
  ...
  private async getPythonDependencies(
    workspace: Workspace,
    dir: string
  ): Promise<string[]> {
    ...
    for (const file of pythonFiles) {
      ...
      const fileContent = await workspace.readFile(filePath);
      imports.push(...this.parsePythonImports(fileContent));
    }
    ...
    const externalImports = uniqueImports.filter((importName) => {
      ...
      return (
        !workspace.exists(maybeLocalPath) &&
        !this.common_native_python_packages.has(importName)
      );
    });
  ...
  private parsePythonImports(fileContent: string): string[] {
    ...
    const multipleFromImportRegex =
      /^from ([a-zA-Z0-9_]+) import (?:[a-zA-Z0-9_ ,]+)$/gm;
