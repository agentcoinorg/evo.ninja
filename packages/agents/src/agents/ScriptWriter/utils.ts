import { Chat, DEFAULT_ADA_CONFIG, OpenAIEmbeddingAPI } from "@/agent-core";
...
export const createScriptWriter = (context: AgentContext): ScriptWriter => {
  ...
  const embedding = new OpenAIEmbeddingAPI(context.env.OPENAI_API_KEY, context.logger, context.chat.tokenizer, DEFAULT_ADA_CONFIG);
